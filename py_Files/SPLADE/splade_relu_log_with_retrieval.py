...
from splade_relu_log import encode_splade
import os
import re
import json
import torch
...
# === Get best SPLADE index for a sentence ===
def find_best_index(sentence, sparse_matrix):
    query_vector = encode_splade(sentence)
    
    if query_vector is None:
        print(f"⚠️ Warning: Could not encode sentence: '{sentence}'")
        return None
    
    # Ensure it's sparse
    query_sparse = query_vector if hasattr(query_vector, 'toarray') else csr_matrix(query_vector)
    
    similarities = sparse_matrix.dot(query_sparse.T).toarray().flatten()
...
sentences = [
    "Do you mean when you shacked up with a slutty hairdresser, but then she dumped you?",
...
# === Run it ===
if __name__ == "__main__":
    splade_index = load_sparse_index(index_path)
    print(f"SPLADE index loaded: shape {splade_index.shape}")

    for sentence in sentences:
        idx = find_best_index(sentence, splade_index)
        if idx is not None:
            print(f"\nSentence: '{sentence}'")
            print(f"→ Matched SPLADE index ID: {idx}")
        else:
            print(f"\nSentence: '{sentence}' → No valid SPLADE index found.")
